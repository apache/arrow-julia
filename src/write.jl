...
function write(file::String, tbl; largelists::Bool=false, compress::Union{Nothing, Symbol}=nothing, denseunions::Bool=true, dictencode::Bool=false, dictencodenested::Bool=false, debug::Bool=false)
    open(file, "w") do io
        write(io, tbl, true, largelists, compress, denseunions, dictencode, dictencodenested, debug)
    end
    return file
end

function write(io::IO, tbl; largelists::Bool=false, compress::Union{Nothing, Symbol}=nothing, denseunions::Bool=true, dictencode::Bool=false, dictencodenested::Bool=false, debug::Bool=false, file::Bool=false)
    return write(io, tbl, file, largelists, compress, denseunions, dictencode, dictencodenested, debug)
end
        ...
        if i == 1
            ...
            if !isempty(cols.dictencodings)
                for de in cols.dictencodings
                    ...
                    put!(msgs, makedictionarybatchmsg(dictsch, (col=de.data,), de.id, false, dictencodenested ? compress : nothing, debug), i)
                end
            end
            put!(msgs, makerecordbatchmsg(sch[], cols, compress, debug), i, true)
        else
@static if VERSION >= v"1.3-DEV"
            Threads.@spawn begin
                try
                    ...
                    if !isempty(cols.dictencodings)
                        for de in cols.dictencodings
                            ...
                            put!(msgs, makedictionarybatchmsg(dictsch, (col=vals,), de.id, true, dictencodenested ? compress : nothing, debug), i)
...
function makerecordbatchmsg(sch::Tables.Schema{names, types}, columns, compress, debug) where {names, types}
    b = FlatBuffers.Builder(1024)
...
function makerecordbatch(b, sch::Tables.Schema{names, types}, columns, compress, debug) where {names, types}
