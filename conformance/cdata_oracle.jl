...
function _oracle_python()
    ...
    isempty(py) && error(
        "ARROW_CDATA_ORACLE_PYTHON is not set: run this suite " *
...
if length(ARGS) >= 1 && ARGS[1] == _CDATA_ORACLE_CHILD
    ...
    function _structwrap(sch::AC.Schema, columns, nrows::Integer)
        ...
        d = AC.ArrayData(
            ...
            nrows,
            [AC.BufferSlice()];
            children=collect(AC.ArrayData, columns),
            nullcount=0,
        )
        return f, d
    end
            ...
            try
                ...
                push!(
                    ...
                    Verdict(
                        ...
                        got == want ? :pass : :fail,
                        got == want ? "" :
                        "ordered metadata sequences differ: $got vs $want",
                    ),
                )
            catch e
